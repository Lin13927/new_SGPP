 ...
 * 将解写入文件
 */
void write_VNS_sol(const Graph &graph, const Solution &csol, char *instancefile, const int &fno) {
    // 获取文件名
    char *graph_name = basename(instancefile);

    // 拼接文件路径
    char rltfile_detail[1000];
    sprintf(rltfile_detail, "./results/VNS_%s_%d_%d", graph_name, graph.k, fno);
...
void VNS_run(char *instancefile, string rltfile, double timelimit)
{
	// 读图
	...
	while(crun < runs)
	{
		...
		verify(graph, bsol);
		write_VNS_sol(graph, bsol, instancefile, seed);

		printf("VNS Round %d: best cost=%d\n", crun, rbcost);
