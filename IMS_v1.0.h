 ...
 * 将解写入文件
 */
void write_IMS_sol(const Graph &graph, const Solution &csol, char *instancefile, const int &fno) {
    // 获取文件名
    char *graph_name = basename(instancefile);

    // 拼接文件路径
    char rltfile_detail[1000];
    sprintf(rltfile_detail, "%sresults/IMS_%s_%d_%d", root_path.c_str(), graph_name, graph.k, fno);
...
void IMS_run(char *instancefile, double timelimit)
{
	...
	while(crun < runs)
	{
		...
		verify(graph, bsol);
		write_IMS_sol(graph, bsol, instancefile, seed);
