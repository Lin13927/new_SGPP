...
#include "IMS_v3.0_decomposition.h"
#elif(isTS)
#include "TS.h"
#elif(isVNS)
#include "VNS.h"
...
int main(int argc, char **argv)
{
	...
	if (tuning)
	{
		...
		srand(seed);
//		string rltfile = "./results/result"; // 没有用
#if(isIMS)
...
//		string rltfile = root_path + "results/result";  // 没有用
		string listfile = "instances_list.txt";  // 待测试的算例列表
//		string path = dir + listfile;
		string path = root_path + listfile; //在服务器上用这个地址
		string *instance_list;
		double *tlist;  // time list
		int *klist;  // k list
		...
		tlist = new double[num_instances]; // 运行时间
		klist = new int[num_instances]; // k
...
//			fin >> target_cost_list[temp++];
		...
		{
			// strncpy是C语言标准库中的字符串处理函数，用于将源字符串的前n个字符复制到目标字符串中。
			// 它与strcpy的区别在于允许指定复制的字符数量，可避免超出目标字符串容量的风险。
