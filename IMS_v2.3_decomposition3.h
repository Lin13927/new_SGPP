...
#define isTrueGamma 1  // 使用gamma总表来获取delta
#define NDBLS_mode1 1  // ND-based_LS模式一：两种邻域都从初始解出发
#define NDBLS_mode2 0  // ND-based_LS模式二：先move，再swap
#define NDBLS_mode3 0  // ND-based_LS模式三：先swap，再move
#define rMS_mode1 0  // 概率控制NBL和BL
#define rMS_mode2 0  // 先NBL再BL
#define rMS_mode3 1  // 先BL再NBL
#define is_swap 1  // 是否用[概率]控制是否要swap，如果为0，则一直会使用swap
...
int sp, wp;
double param_q = 0.01; // 控制swap的参数
double param_b = 0.4; // local_search和biased_local_search的参数
...
void allocate_memory(const Graph &graph)
{
	...
	active_matrix = new int *[graph.k];
	for (int i = 0; i < graph.k; i++)
	{
		active_matrix[i] = new int[graph.k];
	}
}
...
{
	for (int i = 0; i < graph.k; i++)
		memset(active_matrix[i], 1, sizeof(int) * graph.k);
}
...
{
	// 更新行
	memset(active_matrix[pid1], 1, sizeof(int) * graph.k);
	memset(active_matrix[pid2], 1, sizeof(int) * graph.k);

	// 更新列
	for (int i = 0; i < graph.k; i++)
	{
		active_matrix[i][pid1] = 1;
		active_matrix[i][pid2] = 1;
	}
}
...
void read_RH_sol(const Graph &graph, Solution &csol, char *instancefile, const int &fno)
{
	...
	if (tuning)
		...
		// 生成新的前缀zzzzmutex&graph_name&tid（每个时间块文件各自拥有一个互斥文件）
	else
...
void update_Gamma(const Graph &graph, Solution &csol, const Gain_node &gnode)
{
	if (gnode.type == 0)
	{
		...
		for (int clst2 = 0; clst2 < graph.k; clst2++)
		{
			...
			{
				true_gamma[clst2][v1] = 0;
			}
			else
			{
				true_gamma[clst2][v1] = neg_gamma[clst2][v1] - neg_gamma[clst1][v1]
				                        - pos_gamma[clst2][v1] + pos_gamma[clst1][v1];
			}
		}
		...
		{
			int v2 = graph.nodes[v1].edges[i].first;
			int clst1 = csol.ptn[v2];
			for (int clst2 = 0; clst2 < graph.k; clst2++)
			{
				if (clst2 == csol.ptn[v2])
				{
					true_gamma[clst2][v2] = 0;
				}
				else
				{
					true_gamma[clst2][v2] = neg_gamma[clst2][v2] - neg_gamma[clst1][v2]
					                        - pos_gamma[clst2][v2] + pos_gamma[clst1][v2];
				}
			}
		}
#endif
	...
	{
		printf("ccost=%d, vcost=%d", csol.cost, csol.cal_cost(graph));
		fflush(stdout);
		exit(-666);
...
Solution swap_local_search_decomposition(const Graph &graph, const Solution &bsol, clock_t cstime)
{
	...
	while (improved)
	{
		...
		for (int pid1 = 0; pid1 < graph.k; pid1++)
		{
			...
			for (int pid2 = pid1 + 1; pid2 < graph.k; pid2++)
			{
				if (active_matrix[pid1][pid2] == 0)
					continue;

				active_matrix[pid1][pid2] = active_matrix[pid2][pid1] = 0;
				for (const int &vtx1: clst_elems[pid1])
				{
					for (const int &vtx2: clst_elems[pid2])
					{
						...
						{
							best_delta = delta;
							best_gn = Gain_node(vtx1, vtx2, -1, delta, 1);
						}
					}
				}
			}
			...
			if (best_delta < 0)
			{
				int bvtx1 = best_gn.elem1;
				int bvtx2 = best_gn.elem2;
				...
				update_active_matrix(graph, bclst1, bclst2);
				if (csol.cost < rbcost)
				{
					rbcost = csol.cost;
					rbtime = (clock() - start) / static_cast<double>(CLOCKS_PER_SEC);
	...
	while (improved)
	{
		...
		for (int clst1 = 0; clst1 < graph.k; clst1++)
		{
			...
			for (int vtx1: clst_elems[clst1])
			{
				...
				for (int clst2 = 0; clst2 < graph.k; clst2++)
				{
					if (clst1 == clst2 || active_matrix[clst1][clst2] == 0)
						continue;

					active_matrix[clst1][clst2] = 0;
#if (isTrueGamma)
					int delta = true_gamma[clst2][vtx1];
#else
					int delta = neg_gamma[clst2][vtx1] - neg_gamma[clst1][vtx1]
...
/**
 *  基于邻域分解的局域搜索（三种模式：1.两种邻域都从初始解开始跑，2.先N1再N2；3.先N2再N1）
 * @param graph
 * @param csol
 * @param cstime
 * @param tlimit
...
Solution Biased_LS(const Graph &graph, Solution &csol, clock_t cstime, double tlimit)
{
	...
	Solution bsol0 = Solution(biased_local_search1(graph, csol, cstime));
	Solution bsol1 = Solution(biased_local_search2(graph, csol, cstime));
	Solution bsol2 = Solution(biased_local_search3(graph, csol, cstime));
	...
	if (rnd < param_q * K / graph.nnode)
	{
		Solution nsol3 = Solution(swap_local_search(graph, csol, cstime));
		if (nsol3.cost < csol.cost) csol.cpy(nsol3);
		bsol.cpy(csol);
	}
#else
	cal_swap_value(graph, csol); // 更新权重
	bsol.cpy(swap_local_search(graph, bsol, cstime)); // swap的
#endif
